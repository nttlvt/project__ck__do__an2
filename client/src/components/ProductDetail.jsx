...
export const ProductDetail = () => {
  ...
  return (
    <aside
      ...
      }  product-detail flex-col fixed right-0 border border-black rounded-lg fondo`}
    >
      <div className="flex justify-between items-start px-5 mt-5">
        <div>
          ...
          <span className="text-xs">
            {context.showProductDetail?.category?.name}
          </span>
        </div>
        <div>
          <XMarkIcon
      ...
      <div className="flex flex-row justify-between mt-0 mb-5 px-5">
        ...
        <p className="flex justify-end px-6">
          Price:
          <span className="font-medium text-2xl text-red-800 ml-2">
      ...
      <p className="flex flex-col p-6">
        <span className="font-light text-sm">
          {context.showProductDetail?.description?.length > 400
            ? context.showProductDetail?.description.substring(0, 400) + "..."
            : context.showProductDetail?.description}
        </span>
      </p>

      <Link
        to={`/product/${context.showProductDetail?._id}`}
